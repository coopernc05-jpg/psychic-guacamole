...
class RiskManager:
    ...
    def can_open_position(self, position_size: float) -> tuple[bool, str]:
        ...
        if position_size > self.config.max_position_size:
            return (
                False,
                f"Position size ${position_size:.2f} exceeds max ${self.config.max_position_size:.2f}",
            )
        ...
        if self.current_exposure + position_size > self.config.max_total_exposure:
            return (
                False,
                f"Would exceed max exposure ${self.config.max_total_exposure:.2f}",
            )
        ...
        if position_size > self.total_capital:
            return (
                False,
                f"Insufficient capital (need ${position_size:.2f}, have ${self.total_capital:.2f})",
            )
    ...
    def remove_position(self, position_id: str):
        ...
        if position_id in self.positions:
            ...
            logger.info(
                f"Removed position {position_id}, Current exposure: ${self.current_exposure:.2f}"
            )
    ...
    def get_risk_metrics(self) -> Dict[str, float]:
        ...
        exposure_pct = (
            (self.current_exposure / self.total_capital * 100)
