...
class TradeExecutor:
    ...
    def _log_alert(self, scored_opportunity: ScoredOpportunity, position_size: float):
        ...
        logger.info(f"Capital Efficiency: {scored_opportunity.capital_efficiency_score:.2f}")
    ...
    async def _execute_multi_leg(
        self,
        opportunity: 'MultiLegOpportunity',
        position_size: float
    ) -> list[Trade]:
    ...
    async def _execute_correlated_events(
        self,
        opportunity: 'CorrelatedEventsOpportunity',
        position_size: float
    ) -> list[Trade]:
    ...
    async def _place_order(
        ...
        if self.config.dry_run:
            logger.info(f"DRY RUN: Would place {side.value} order for {outcome} on {market_id[:8]}... at {price:.3f} for ${size:.2f}")
            return None
        ...
        try:
            ...
            logger.info(f"Executed trade: {side.value} {size:.2f} {outcome} @ {price:.3f} on {market_id[:8]}...")
