...
class PolymarketArbitrageBot:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.ws_client = WebSocketClient(self.config) if self.config.websocket_enabled else None
    ...
    async def _fetch_markets(self):
        ...
        try:
            ...
            if isinstance(self.config.markets_to_monitor, str) and self.config.markets_to_monitor == "all":
    ...
    async def _process_opportunities(self, scored_opportunities: list):
        ...
        for opp_idx, position_size in allocation.items():
            ...
            if self.config.mode == "auto_trade":
                ...
                if can_open:
                    trades = await self.executor.execute_opportunity(scored_opp, position_size)
    ...
    def _log_iteration_metrics(self):
        ...
        logger.info(f"Capital: ${risk_metrics['total_capital']:.2f}, "
