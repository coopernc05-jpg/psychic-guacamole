...
class OpportunityLogger:
    ...
    def log_opportunity(self, scored_opportunity: ScoredOpportunity):
        ...
        log_file = self.log_dir / f"opportunities_{datetime.now().strftime('%Y%m%d')}.jsonl"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_score = sum(opp["score"] for opp in self.opportunities) / len(self.opportunities)
        avg_profit = sum(opp["profit_score"] for opp in self.opportunities) / len(self.opportunities)
...
class ExecutionLogger:
    ...
    def log_position_close(self, position: Position):
        ...
        entry = {
            ...
            "hold_duration_hours": (position.exit_time - position.entry_time).total_seconds() / 3600 if position.exit_time else None
        }
    ...
    def get_position_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate": len(winning) / len(self.positions) * 100 if self.positions else 0,
            "avg_return_pct": sum(p.get("return_pct", 0) for p in self.positions) / len(self.positions) if self.positions else 0
        }
