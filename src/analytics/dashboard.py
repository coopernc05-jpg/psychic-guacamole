...
class Dashboard:
    ...
    def _setup_routes(self):
        ...
        def get_logs():
            ...
            if log_dir.exists():
                log_files = sorted(log_dir.glob("*.jsonl"), reverse=True)[:3]  # Last 3 days
        ...
        def health_check():
            ...
            return jsonify({
                ...
                'uptime_seconds': (datetime.now() - self.performance_tracker.equity_curve[0][0]).total_seconds()
            })
        ...
        def config():
            ...
            if request.method == 'POST':
                ...
                return jsonify({'status': 'success', 'message': 'Configuration update not implemented'})
            else:
