...
def create_test_market(market_id: str, question: str, yes_price: float, no_price: float, 
                       yes_ask: float = None, no_ask: float = None,
                       yes_bid: float = None, no_bid: float = None) -> Market:
...
class TestMultiLegStrategy:
    ...
    def test_detect_multi_leg_opportunity(self):
        ...
        markets = [
            create_test_market(f"market_{i}", f"Election outcome {i}", 
                             0.30 + i*0.1, 0.70 - i*0.1)
            for i in range(5)
        ]
...
class TestCorrelatedEventsStrategy:
    ...
    def test_groups_by_category(self):
        ...
        sports_market = create_test_market(
            "sports_1", "Sports question", 0.50, 0.50
        )
...
class TestStrategyIntegration:
    ...
    def test_all_strategies_on_same_markets(self):
        ...
        markets = [
            create_test_market(f"market_{i}", f"Question {i}", 
                             0.40 + i*0.05, 0.60 - i*0.05)
            for i in range(10)
        ]
