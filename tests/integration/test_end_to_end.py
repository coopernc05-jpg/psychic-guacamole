...
def create_test_config():
    ...
    config.mode = 'alert'
    config.strategies = ['cross_market', 'yes_no_imbalance', 'multi_leg', 'correlated_events']
    return config
...
class TestEndToEndWorkflow:
    ...
    async def test_full_workflow_integration(self, components, config):
        ...
        position_size = allocator.calculate_position_size(
            expected_profit_percentage=best_opportunity.opportunity.expected_profit / best_opportunity.opportunity.required_capital * 100 if best_opportunity.opportunity.required_capital > 0 else 0,
...
class TestSystemIntegration:
    ...
    def test_component_compatibility(self):
        ...
        assert all([detector, scorer, allocator, risk_manager, tracker, opp_logger, exec_logger])
